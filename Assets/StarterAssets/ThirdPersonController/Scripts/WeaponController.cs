...
public class WeaponController : MonoBehaviour
{
  ...
  void Update()
  {
    if (tpc != null && tpc.isCombatMode)
    {
      ...
      {
        // Buffer light attack
        if (input.lightAttack && !bufferedLightAttack)
        {
          Debug.Log("Buffered attack executed");
          bufferedLightAttack = true;
          Invoke("ClearBuffer", bufferWindow);
  ...
  void ResetAttack()
  {
    ...
    // Check the memory! Did they click while we were swinging?
    if (bufferedLightAttack)
    {
      Debug.Log("Buffered attack executed");
      playerAnimator.ResetTrigger("Attack");
      bufferedLightAttack = false;
      ExecuteAttack(0, lightAttackTwist); // Instantly swing again!
    }
  }
